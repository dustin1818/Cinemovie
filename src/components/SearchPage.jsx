...
const SearchPage = ({ toggleBtn2, options, useNavigate, setMovieSeriesID }) => {
  const [newMovie, setNewMovie] = useState(null);
  const [pageCount, setPageCount] = useState(0);
  const ref = useRef(null);

  const searchItem = localStorage.getItem("SearchVar");
  ...
  useEffect(() => {
    const fetchMovie = async (currentPage) => {
      try {
        let API_NEW_POPULAR =
          toggleBtn2 === true
            ? `https://api.themoviedb.org/3/search/movie?query=${searchItem}&include_adult=false&language=en-US&page=1`
            : `https://api.themoviedb.org/3/search/tv?query=${searchItem}&include_adult=false&language=en-US&page=1`;
  ...
  const getNextPage = async (currentPage) => {
    let API_NEW_POPULAR =
      toggleBtn2 === true
        ? `https://api.themoviedb.org/3/search/movie?query=${searchItem}&include_adult=false&language=en-US&page=${currentPage}`
        : `https://api.themoviedb.org/3/search/tv?query=${searchItem}&include_adult=false&language=en-US&page=${currentPage}`;
    const response = await fetch(API_NEW_POPULAR, options);
    const data = await response.json();
    return data;
  };
