...
const Trending = ({ toggleBtn2, options, useNavigate, setMovieSeriesID }) => {
  ...
  useEffect(() => {
    const fetchMovie = async (currentPage = 1) => {
      try {
        let API_NEW_TRENDING = toggleBtn2
          ? `https://api.themoviedb.org/3/trending/all/day?language=en-US&page=${currentPage}`
          : `https://api.themoviedb.org/3/trending/all/day?language=en-US&page=${currentPage}`;
        const response = await fetch(API_NEW_TRENDING, options);
  ...
  const getNextPage = async (currentPage = 1) => {
    let API_NEW_TRENDING = toggleBtn2
      ? `https://api.themoviedb.org/3/trending/all/day?language=en-US&page=${currentPage}`
      : `https://api.themoviedb.org/3/trending/all/day?language=en-US&page=${currentPage}`;
    const response = await fetch(API_NEW_TRENDING, options);
